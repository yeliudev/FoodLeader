...
Page({
  ...
  getDeletedMenuList: function() {
    var that = this
    wx.request({
      url: config.service.getDeletedMenuListUrl,
      method: 'GET',
      header: {
  ...
  onRecoverTap: function() {
    ...
    if (list.length) {
      wx.request({
        url: config.service.recoverMenuUrl,
        method: 'GET',
        header: {
          skey: app.globalData.skey
        },
        ...
        success: res => {
          if (res.data.success) {
            ...
            util.showModal('恢复失败', res.data.errMsg, true)
            wx.startPullDownRefresh()
          }
        },
        fail: error => {
          util.showModal('恢复失败', error, true)
        }
      })
    } else {
      util.showModal('提示', '请先选择菜单')
    }
  },
